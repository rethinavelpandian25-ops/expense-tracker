...
def add_performance_headers(response):
    # Static assets are fingerprinted by deployment/builds in practice; cache
    # them aggressively while keeping HTML/API responses dynamic.
...
class Transaction(db.Model):
    ...
    txn_date = db.Column(db.Date, nullable=False, default=date.today, index=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    ...
    def circular_avatar_png():
        ...
        if current_user.profile_pic and current_user.profile_pic.startswith("data:image/"):
            try:
                ...
                src = PILImage.open(BytesIO(raw)).convert("RGBA")
                side = min(src.size)
                left = (src.width - side) // 2
                top = (src.height - side) // 2
                src = src.crop((left, top, left + side, top + side)).resize((size, size), PILImage.Resampling.LANCZOS)
            except Exception:
                src = None
        else:
            src = None
        if src is None:
            src = PILImage.new("RGBA", (size, size), "#6366f1")
            draw = ImageDraw.Draw(src)
            letter = (current_user.username or "?")[0].upper()
            try:
                font = __import__("PIL").ImageFont.truetype("DejaVuSans-Bold.ttf", 92)
            except Exception:
                font = None
            bbox = draw.textbbox((0, 0), letter, font=font)
            draw.text(((size-(bbox[2]-bbox[0]))/2, (size-(bbox[3]-bbox[1]))/2-8), letter, fill="white", font=font)
        mask = PILImage.new("L", (size, size), 0)
        ImageDraw.Draw(mask).ellipse((0, 0, size-1, size-1), fill=255)
        src.putalpha(mask)
        out = BytesIO(); src.save(out, format="PNG"); out.seek(0)
        return out

    avatar = RLImage(circular_avatar_png(), width=22*mm, height=22*mm)
...
def run_startup_migrations():
    ...
    with db.engine.begin() as conn:
        ...
        # Helpful for report/transaction APIs as the history grows. SQLite and
        # PostgreSQL both support these simple indexes.
        conn.execute(text('CREATE INDEX IF NOT EXISTS ix_transaction_user_date ON "transaction" (user_id, txn_date)'))
        conn.execute(text('CREATE INDEX IF NOT EXISTS ix_transaction_user_type ON "transaction" (user_id, type)'))
